...
def evaluate():
    ...
    video_to_test = sftp.list_dir_remote(PATH_TO_TIME_ON_TASK_VIDEO_TO_TEST)
    ...
    for video in video_to_test:
        ...
        df_video = sftp.read_remote_df(os.path.join(PATH_TO_TIME_ON_TASK_VIDEO_TO_TEST,video,"/"+video+".csv"))
        ...
        sftp.save_remote_df(path_to_csv, df, index=False)

    sftp.save_remote_df(os.path.join(PATH_TO_RESULTS_PREDICTIONS, "metrics.csv"), df_evaluate_metrics)
    del sftp
