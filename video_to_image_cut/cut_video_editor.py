...
def video_cut(path_to_video):
    ...
    for index, windows_to_cut in enumerate(windows_to_cut_frame):
        ...
        size = (width,height)
        out = cv2.VideoWriter("data/stage_data_out/image_for_irba/"+video_name+"/"+video_name+"_"+str(random_sequence_order[index])+".avi",cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
...
file_path = file_path + ["/mnt/7b914d1c-f145-4023-9f2f-2cd288d7db76/DESFAM-F/"+video_path for video_path in subject_list_vendredi + subject_list_lundi if os.path.isfile("/mnt/7b914d1c-f145-4023-9f2f-2cd288d7db76/DESFAM-F/"+video_path) == True]
print(file_path)
